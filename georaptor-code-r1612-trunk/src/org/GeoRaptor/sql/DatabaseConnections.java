...
public class DatabaseConnections {
    ...
    public javax.swing.DefaultComboBoxModel getComboBoxModel(String _defaultConnName) {
        if ( this.cache == null || this.cache.size()==0 ) {
        ...
        while (iter.hasNext()) {
            DatabaseConnection dbConn = iter.next();
            if (dbConn.isOpen()) {
                cm.addElement(dbConn.getDisplayName());
            }
        }
        return cm;
    }
    ...
    {
        return new JComboBox(this.getComboBoxModel(null));
    }
    ...
    public void removeConnection(String _connName) {
        if ( this.connectionExists(_connName) ) {
            this.cache.remove(this.cache.get(_connName));
        }
    }
