 ...
 * @tobedone 1. True, array loading to increase performance.
 *           2. Any action in attribute table (eg tick box in column lisEist - Load?), keep FID selection if exists.
 */
public class ShapefileLoad extends javax.swing.JDialog 
{
    ...
    private static enum LOAD_TYPE { NEW, INSERT, UPDATE };
    private LOAD_TYPE load_operation = LOAD_TYPE.NEW;

    private DefaultListModel ShapefileListModel;
    private SHPTableModel shptm;
    
    /**
     * @constructor ShapefileLoad
     * @author Simon Greener, June 10th 2010
     *          - John O'Toole comment:
     *              "It's a modal window & probably shouldn't be. "
     */
        ...
        fcShapefile.setCurrentDirectory(new java.io.File("C:\\"));
            ...
            layout.setVerticalGroup(
                ...
                .addGroup(layout.createSequentialGroup()
                    ...
                    .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED)
                    .addComponent(pnlTableProperties, javax.swing.GroupLayout.PREFERRED_SIZE, 190, javax.swing.GroupLayout.PREFERRED_SIZE)
                    .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED)
                    .addGroup(layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
                        .addGroup(layout.createSequentialGroup()
    ...
    private void btnAddShpActionPerformed(java.awt.event.ActionEvent evt) 
    {//GEN-FIRST:event_btnAddShpActionPerformed
        addShapefileToList();
    ...
    private void btnHelpActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnHelpActionPerformed
        ...
        HtmlHelp hh = new HtmlHelp(this.propertyManager.getMsg("HELP_TITLE"),
                                   ...
                                   String.format(this.propertyManager.getMsg("HELP_CONTENT"),
                                                 ...
                                                 image_url_4,image_url_5,image_url_6,image_url_6));
        hh.display();
    ...
    public void initialise() {
        ...
        DatabaseConnection dbConn = DatabaseConnections.getInstance().findDisplayName((String)this.cmbConnections.getItemAt(0));
        ...
        this.ShapefileListModel = new DefaultListModel();
    ...
    public org.GeoRaptor.SpatialView.SpatialViewPanel.LayerReturnCode initialise(String _schemaName, String _objectName, String _columnName, String _connName) {
        ...
        try {
            ...
            if (Strings.isEmpty(_columnName) && metaEntries.size() > 1) {
                ...
                String value =
                    (String)JOptionPane.showInputDialog(null, propertyManager.getMsg("ADD_LAYER_SELECT_COLUMN"), MainSettings.EXTENSION_NAME, JOptionPane.QUESTION_MESSAGE, null,
                                                        // Use default icon
        ...
        // Load connection pick list....
        this.cmbConnections.setEnabled(false);
        javax.swing.DefaultComboBoxModel cm = new javax.swing.DefaultComboBoxModel();
        ...
        this.cmbConnections.setSelectedIndex(0); // This fires setting of this.conn and userName() via cmbConnectionsActionPerformed
        if (conn == null) {
            this.setAlwaysOnTop(false);
            JOptionPane.showMessageDialog(this, propertyManager.getMsg("SHPFILE_IMPORTER_NO_OPEN_CONN_ERROR"));
            this.setAlwaysOnTop(true);
        ...
        try {
            this.shptm.loadOraAttributes(MetadataTool.getColumnsAndTypes(
                                       conn,
    ...
    private OracleConnection getConnection(String _connName, boolean _display) {
        ...
        if (dbConn == null) {
            this.setAlwaysOnTop(false);
            JOptionPane.showMessageDialog(null, "Could not get Connection for " + _connName, Constants.GEORAPTOR + " - " + this.getClass().getName(), JOptionPane.ERROR_MESSAGE);
            this.setAlwaysOnTop(true);
        }
    ...
    private void loadCmbConnections() {
        ...
        JComboBox jb = DatabaseConnections.getInstance().getConnectionsCombo();
    ...
    private void processSRIDAction() {
        ...
        boolean status = sp.initialise(getConnection(this.cmbConnections.getSelectedItem().toString(), true) /*display*/, this.tfSRID.getText());
        if (status == true) {
    ...
    public ListModel getShapefileModel() {
        return this.lstShapefiles.getModel();
    }

    public int getSRIDAsInteger() {
        return (Strings.isEmpty(this.tfSRID.getText()) || this.tfSRID.getText().equals("NULL")) ? 0 : Integer.valueOf(this.tfSRID.getText()).intValue();
    }
    ...
    public String getSRID() {
        if (Strings.isEmpty(this.tfSRID.getText()) || this.tfSRID.getText().equalsIgnoreCase("NULL"))
            return "NULL";
        try {
            int checkInt = Integer.parseInt(this.tfSRID.getText());
            return this.tfSRID.getText();
        } catch (Exception e) {
            this.setAlwaysOnTop(false);
            JOptionPane.showMessageDialog(null, this.propertyManager.getMsg("SHPFILE_IMPORTER_SRID_ERROR", this.tfSRID.getText()), this.TITLE, JOptionPane.ERROR_MESSAGE);
            this.setAlwaysOnTop(true);
        }
        return "NULL";
    }
    ...
    private void addShapefileToList() {
        ...
        if (returnVal == JFileChooser.APPROVE_OPTION) {
            ...
            if (pos > 0) {
                for (int i = 0; i < pos; i++)
                    if (!Strings.isEmpty(this.ShapefileListModel.getElementAt(i).toString()) && this.ShapefileListModel.getElementAt(i).equals(fileName)) {
                        Toolkit.getDefaultToolkit().beep();
                        return;
                    }
            } else {
        ...
        if (!Strings.isEmpty(errorMessage)) {
            ...
            JOptionPane.showMessageDialog(null, errorMessage + "\n" +
                    fcShapefile.getSelectedFile().getAbsolutePath(), this.TITLE, JOptionPane.INFORMATION_MESSAGE);
            this.setAlwaysOnTop(true);
        }
    }
     ...
     * @param _colValue
     * @return boolean
     * @author Simon Greener, May 2010, Original Coding
     *          Simon Greener, May 26th 2010, Fixed bug with incorrect detection of selected item within list.
     *          Simon Greener, May 27th 2010, Attempt to support user entered FID across shapefile deletes and table refreshes.
     */
    ...
    /**
     * @method refreshFIDs
     * @precis Rebuilds FID combo box adding in new attributes in response to changes in number of shapefiles or Oracle data types entered
     * @author Simon Greener, 20th May 2010, Original Coding
     *          Simon Greener, 27th May 2010, Attempt to support user entered FID values
     */
     ...
     * @author Simon Greener, 20th May 2010, Original Coding
     *          Simon Greener, 27th May 2010, Simplified in attempt to support user entered FID values
     */
    protected void refreshCreateTable() {
        ...
        for (int i = 0; i < shptm.getRowCount(); i++) {
            ...
            //
            sql += "  " + (shptm.getValueAt(i, ora_col).toString() + padString).substring(0, 34) + shptm.getValueAt(i, ora_typ).toString() + ",\n";
        }
        // Add geometry
        //
    ...
    private void loadAllShapefiles() {
        ...
        if (shpCount > 0) {
            int roundNumber = this.cmbDecimalPlaces.getSelectedItem().toString().equals(this.sNone) ? -1 : Integer.valueOf(this.cmbDecimalPlaces.getSelectedItem().toString());
            DatabaseConnection dbConn = DatabaseConnections.getInstance().findConnectionName(this.connName);
            if (dbConn == null) {
                return;
            }
            ...
            batchLoader work =
                new batchLoader(dbConn.getConnectionName(), 
                                dbConn.getUserName(), 
            ...
            progress.start("Loading " + shpCount + " shapefile" + ((shpCount == 1) ? "" : "s") + ", please wait...",
                           ...
                           1, 
                           this.totalRecords);
            /*progress.start("Loading " + shpCount + " shapefile" +
                           ((shpCount == 1) ? "" : "s") +
                           ", please wait...", this.ShapefileListModel.getElementAt(0).toString());
            */
            ...
            if (!Strings.isEmpty(work.errorMessage)) {
                ...
                JOptionPane.showMessageDialog(null,
                                              (work.errorCount > 0) 
                                              ? String.valueOf(work.errorCount) + " record(s) not converted.\n" 
                                              : "" + String.valueOf(work.successCount) + " record(s) converted out of " +
                                              String.valueOf(work.successCount + work.errorCount) + " processed.\n" +
                        "Processing time: " + Tools.milliseconds2Time(timeDiff), this.TITLE, JOptionPane.INFORMATION_MESSAGE);
            }
            this.setAlwaysOnTop(true);
            progress = null;
            work = null;
    ...
    class batchLoader implements Runnable {
        ...
        private ListModel ShapeFileListModel;
        private String srid;
        ...
        public batchLoader(String _connName, 
                           String _user, 
                           String _tableName, 
                           String _userFID, 
                           String _geomCol, 
        ...
        private void displayProgress() {
             ...
             *
             * progressText - The text to display just above the progress indicator.
             *                If a null string or zero-length string is passed in the text will not be updated.
             * stepText - The text to display just below the progress indicator.
             *                If a null string or zero-length string is passed in the text will not be updated.
             */
            ...
            if (processedRecords > 0 && this.totalRecords != 0) {
                ...
                //
                java.util.Date estimatedFinish = new java.util.Date(this.startTime.getTime() + (timeDiff * totalRecords / processedRecords));
                ...
                stepText = String.format("Processing %s (%s of %s)", 
                                         fileName, 
                                         String.valueOf(this.currentShapefile + 1).trim(), 
                                         String.valueOf(this.shapeFileCount).trim());
            }
        ...
        private boolean isOracleNumber(String _oracleDataType) {
            ...
            return (oracleDataType.startsWith("NUMBER") || oracleDataType.startsWith("BINARY_FLOAT") || oracleDataType.startsWith("BINARY_DOUBLE") || oracleDataType.startsWith("NUMERIC") ||
                    oracleDataType.startsWith("DECIMAL") || oracleDataType.startsWith("FLOAT") || oracleDataType.startsWith("REAL") || oracleDataType.startsWith("DOUBLE PRECISION"));
        }

        private boolean isOracleInteger(String _oracleDataType) {
            return (_oracleDataType.equalsIgnoreCase("INTEGER") || _oracleDataType.equalsIgnoreCase("INT") || _oracleDataType.equalsIgnoreCase("SMALLINT"));
        }
         ...
         *
         *         For example, if you let Oracle do an implicit conversion of 1234.56789
         *         into a number(7,3) it will work. But if it was number(6,3) it won't as
         *         there aren't enough precision digits for the 1234. The error message would be:
         *
         *         SQL Error: ORA-01438: value larger than specified precision allowed for this column
         *         01438. 00000 -  "value larger than specified precision allowed for this column"
         *
         *         Similarly, if the string contained non-numeric items then these need to be trapped.
         *
         * @param _oracleDataType
         * @param _number
         * @param roundLeft  -- Divide precision by power(10,maxPossible) - true - or Mod(precisionValue,power(10,maxPossible) false
         * @return String
        ...
        private String fixNumber(String _oracleDataType, String _number, boolean roundLeft) {
            ...
            try {
                ...
                //
                int leftSize = (intVal != null) ? String.valueOf(Math.abs(intVal)).length() : String.valueOf(Math.abs(dblVal.intValue())).length();
                double leftVal = roundLeft ? Math.pow(10, leftSize - (precision - scale)) : Math.pow(10, precision - scale);
                leftVal = leftVal < 1 ? 1 : leftVal;
                String fmt = "%" + precision + ((scale == 0) ? ".0" : "." + String.valueOf(scale)) + "f";
                result =
                        roundLeft ? String.format(fmt, (intVal == null ? dblVal : Double.valueOf(intVal)) / leftVal).trim() : String.format(fmt, (intVal == null ? dblVal : Double.valueOf(intVal)) % leftVal).trim();
                return result;
            } catch (Exception e) {
                LOGGER.warning("Problem fixing number " + _number + " for " + _oracleDataType + " ==> " + e.getMessage());
            }
            return _number;
        }
        ...
        private String getLayerGType(int _shpFileType, String _existingLayerGType) {
            ...
            switch (_shpFileType) {
                ...
                case ShapefileReaderJGeom.AV_POINTZ:
                    {
                        layerGType = (Strings.isEmpty(_existingLayerGType) ? "POINT" : (_existingLayerGType.indexOf("POINT") != -1 ? "MULTIPOINT" : "POINT"));
                        break;
                    }
            ...
            return Strings.isEmpty(_existingLayerGType) ? layerGType : (_existingLayerGType.equals(layerGType) ? layerGType : "COLLECTION");
        }
        ...
        private void updateMetadata(ShapefileReaderJGeom _sfh, 
                                                 boolean _z, 
                                                 boolean _m) 
        {
            ...
            // Make sure a master record exists for this shapefile set (based on target table/column)
            //
            if (this.me == null)
                me = new MetadataEntry(this.userName, this.tableName, this.geometryColumn, this.srid);
        ...
        private int getGType(JGeometry _geom) {
            return ((_geom.getDimensions() * 1000) + ((_geom.isLRSGeometry() && _geom.getDimensions() == 3) ? 300 : ((_geom.isLRSGeometry() && _geom.getDimensions() == 4) ? 400 : 0)) +
                    _geom.getType());
        }
         ...
         * @author Simon Greener, May 2010
         * @decriptions Loads a shapefile into a newly created table
         *              Shapefile can be one of a batch and may have
         *              attributes that are not to be loaded, or for which no value
         *              exists in the oracle table as column may come from another table.
         */
        private void loadShapefile(String _shapefileName, 
                                   String _idName, 
                                  boolean _fidProcessing) 
        {
            ...
            // Get layerGType (which the function "unions" together with the existing global variable
            //
            this.layerGType = getLayerGType(shpFileType, this.layerGType);

            // Get DBF data for conversion
            // 1. Get actual DBF names and data types irrespective as to what user wants to load
            // 2. Oracle Utility's hash table creates keys based on lower case versions of original DBF (which it calls its "Oracle COlumn Names")
            //
            ...
            // 1. Check if user fid has been requested and place at front of column and parameter (values) lists
            //
            int colsToStore = 0;
            String insertCols = " (";
            String params = "(";
            ...
            // 2. Get Oracle Columns names for those rows in shptm that are related to this shapefile and are to be loaded
            //
            ...
            if (shpFileType == 5 || shpFileType == 15 || shpFileType == 25) {
                params +=
                        ...
                        ((shpFileType == 15) ? "," + MetadataTool.createDimElement("Z", "0", "1", String.valueOf(this.migrateTolerance)) : "") +
                        ((shpFileType == 25) ? "," + MetadataTool.createDimElement("M", "0", "1", String.valueOf(this.migrateTolerance)) : "") + "))";
            } else {
                params += "?";
            }
            params += ")";
            ...
            for (int rec = 1; rec <= numRecords; rec++) 
            {
                try {
                    ...
                    //
                    ht = ShapefileFeatureJGeom.fromRecordToFeature(dbfr, sfh, fieldTypes, numFields, (rec - 1), getSRIDAsInteger());
                } catch (IOException ioe) {
                ...
                if (_fidProcessing) {
                    ...
                    if (userFID) {
                        ...
                        try {
                            ...
                            LOGGER.warning("Record# " + rec + " ERROR converting " + oracleColumnName + "(" + oracleDataType + ") = " + ht.get(oracleColumnName).toString() + sqle.getMessage());
                        }
                    }
                }
                ...
                while (setIter.hasNext()) 
                {
                    try {
                        ...
                        // columnBeingProcessed only includes columns in the target table that are in this shapefile
                        ...
                        if (_fidProcessing && (!userFID) && oracleColumnName.equalsIgnoreCase(_idName)) {
                            ...
                            ps.setDouble(parameterIndex++, Double.valueOf(ht.get(oracleColumnName).toString()).doubleValue());
                ...
                try {
                    ...
                    geometrySTRUCT = JGeometry.store(conn, geom);
                } catch (Exception e) {
            ...
            try {
                dbfr.closeDBF();      dbfr = null; // displose of handle
                sfh.closeShapefile(); sfh = null;
            } catch (IOException ioe) {
                this.errorMessage = ioe.getMessage();
            }
            try {
    ...
    public static String mapDBFTypeToOracle(char _dbfType, int _length, int _decPlaces) {
        ...
        switch (_dbfType) {
            ...
            case 'N':
                ...
                if (_decPlaces != 0) {
                    int decPlaces = (int)((_length>38) ? (38.0*(((double)_decPlaces)/((double)_length))) : _decPlaces);
    ...
    //
    public static enum columns { DBF_COLUMN_NAME, 
                                 ORACLE_COLUMN_NAME, 
                                 ORACLE_DATA_TYPE, 
                                 IS_LOAD, 
                                 DBF_DATA_TYPE };
    ...
    class SHPTableModel extends AbstractTableModel {
        public String[] headers = new String[] { "DBF Column", 
                                                 "Oracle Column", 
                                                 "Data Type", 
                                                 "Load?" };
        Class[] types = new Class[] { java.lang.String.class, java.lang.String.class, java.lang.String.class, Boolean.class };
        ...
        public int getColumnPosition(ShapefileLoad.columns _col) {
            int position = -1;
            switch (_col) {
                case DBF_COLUMN_NAME:    position = dbf_col;  break;
                case ORACLE_COLUMN_NAME: position = ora_col;  break;
                case ORACLE_DATA_TYPE:   position = ora_typ;  break;
                case IS_LOAD:            position = is_load;  break;
                case DBF_DATA_TYPE:      position = dbf_typ;  break;
            }
            return position;
        }
        ...
        public String getColumnType(int row) {
            return (String)((Object[])cache.elementAt(row))[ora_typ];
        }

        public String getDBFType(int row) {
            return (String)((Object[])cache.elementAt(row))[dbf_typ];
        }
        ...
        public void setValueAt(Object value, int row, int col) {
            ...
            Object[] obj = (Object[])cache.elementAt(row);
            obj[col] = value;
        ...
        public void loadShpAttributes(String shapefileName) 
        throws Exception
        {
            ...
            for (int field = 1; field <= numFields; field++) {
                try {
                    ...
                    // We assume here that the shapefile being loaded does not have duplicate DBF attribute names
                    //
                    if (!valueExists(dbFld.getName())) {
                        Object[] record = new Object[5];
                        record[dbf_col] = dbFld.getName(); 
                        record[ora_col] = dbFld.getName().toUpperCase(Locale.getDefault());
                        record[ora_typ] = mapDBFTypeToOracle(dbFld.getType(), dbFld.getLength(), dbFld.getDecimalPositionCount());
                        record[is_load] = Boolean.TRUE;
                        record[dbf_typ] = String.valueOf(dbFld.getType());
                        // add Metadata to internal cache
                        //
                        // LOGGER.info("about to add field " + field + " out of " + numFields + ": " + record[ora_col] + " " + record[ora_typ]);
                        this.addRow(record);
                        fieldCount++;
                    }
                } catch (Exception e) {
        ...
        /**
         * When edit a cell eg Oracle column name or data type, we need to refresh the table in the SQL text area
         */
